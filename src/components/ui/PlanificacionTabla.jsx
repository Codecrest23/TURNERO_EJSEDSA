...
export default function PlanificacionTabla({
  ...
  // alturas
  const rowH = isMobile ? 33 : 36;
  const headerH = isMobile ? 32 : 35;
  const monthH = isMobile ? 24 : 26;

  // anchos (clave)
  const stickyColW = isMobile ? 118 : 230;   // 👈 baja fuerte la fija
  const dayColW = isMobile ? 48 : 80;        // 👈 días más angostos

  // alto visible
  const maxVisibleRows = isMobile ? 14 : 15;
  ...
  ///PARA MOBILE///
  function useIsMobile(breakpointPx = 640) { // tailwind sm
        ...
        <div className="overflow-y-auto  scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
          <table className="border-collapse" style={{ minWidth: tableWidth }}>
            <thead className="bg-gray-100">
              ...
              <tr >
                <th
                  className="sticky left-0 bg-gray-100 border-b border-r border-gray-200 z-[70]"
                  style={{ width: stickyColW, minWidth: stickyColW, height: headerH, top: monthH }}
                >
                  Localidad / Empleado
                </th>
                ...
                {days.map((d) => (
                  <th
                    key={toYMD(d)}
                    className=" bg-gray-100 border-b border-r border-gray-200 px-2 text-center text-xs font-semibold whitespace-nowrap sticky  z-[60]"
                    style={{ width: dayColW, minWidth: dayColW, height: headerH, top: monthH }}
                  >
                    {dayLabel(d)}
                  </th>
                ))}
                ...
                return (
                  <Fragment key={g.locId}>
                    ...
                    <tr className="bg-gray-100/60">
                      ...
                      >
                        <span className="sticky left-1 z-10 px-1 z-[40]">
                          <button
                            type="button"
                      ...
                      g.empleados.map((e) => {
                        ...
                        return (
                          <tr key={`${g.locId}-${empId}`} className="hover:bg-gray-100/40">
                            <td
                              className={`sticky left-0 z-10 bg-white border-b border-r border-gray-200 px-2
              ${isMobile ? "text-xs" : "text-sm"}`}
